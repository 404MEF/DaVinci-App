...
namespace Davinci.Api
{
    public sealed class DavinciApi
    {
        ...
        //const string BASE_URL = @"http://davinciapi-env.dmti3mhyzz.eu-west-1.elasticbeanstalk.com/api";
        ...
        private static async Task<T> DeleteAsync<T>(string resource, string jsonBody = "", HttpBasicAuthenticator authenticator = null, string token = "") where T : BaseApiModel, new()
        {
            client.Authenticator = authenticator;
        ...
        private static async Task<T> PutAsync<T>(string resource, string jsonBody = "", HttpBasicAuthenticator authenticator = null, string token = "") where T : BaseApiModel, new()
        {
            client.Authenticator = authenticator;
        ...
        public static async Task<BaseApiModel> DeleteAccount()
        {
            const string resource = @"account/";

            return await DeleteAsync<BaseApiModel>(resource, token: Token.value);
        }

        public static async Task<BaseApiModel> ChangeAccount(string newUsername, string newPassword, string newEmail)
        {
            const string resource = @"account/";

            string body = String.Empty;

            if (string.IsNullOrEmpty(newUsername) == false)
            {
                body = JsonConvert.SerializeObject(new { username = newUsername });
            }
            else if (string.IsNullOrEmpty(newPassword) == false)
            {
                body = JsonConvert.SerializeObject(new { password = newPassword });
            }
            else if (string.IsNullOrEmpty(newEmail) == false)
            {
                body = JsonConvert.SerializeObject(new { email = newEmail });
            }
            return await PutAsync<BaseApiModel>(resource, body, token: Token.value);
        }
        #endregion
        #region Feed Methods
        ...
        public static async Task<ProfilePageModel> GetProfilePosts()
        {
            const string resource = @"feed/profile";

            return await GetAsync<ProfilePageModel>(resource, Token.value);
        }
